...
async function playMusic(channel) {
    if (!channel) return console.log('Ses kanalını bulamadım!');
    ...
    function playNextSong() {
        if (queue.length === 0) {
            console.log('🎵 Kuyruktaki tüm şarkılar çalındı! Tekrar başlatılıyor...');
            queue.push('music.mp3', 'music2.mp3', 'music3.mp3'); // Şarkı listesini başa al
        }

        const nextSong = queue.shift(); // Kuyruktan sıradaki şarkıyı al
        if (!fs.existsSync(nextSong)) {
            console.log(`⚠️ Şarkı bulunamadı: ${nextSong}`);
            playNextSong(); // Şarkı yoksa sonraki çal
            return;
        }

        const resource = createAudioResource(nextSong);
        player.play(resource);
        console.log(`🎶 Şimdi çalıyor: ${nextSong}`);
    }

    player.on(AudioPlayerStatus.Idle, playNextSong);
    connection.subscribe(player);

    playNextSong(); // İlk şarkıyı başlat
}

// Bot açılınca müzik çalmaya başlasın
client.on('ready', async () => {
    console.log(`${client.user.tag} aktif!`);

    const guild = client.guilds.cache.get('1345773955620929566'); // Sunucu ID
    if (!guild) return console.log('Sunucu bulunamadı!');

    const channel = guild.channels.cache.get('1345887623301828640'); // Ses kanalı ID
